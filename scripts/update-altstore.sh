...
IPA="release-assets/blox-ios-arm64-unsigned.ipa"
if [ ! -f "$IPA" ]; then
...
VERSION="${TAG#v}"
SIZE=$(stat -c%s "$IPA")
DATE=$(gh release view "$TAG" -R justacalico/blox --json publishedAt -q .publishedAt 2>/dev/null || date -u +%Y-%m-%dT%H:%M:%SZ)
NOTES=$(gh release view "$TAG" -R justacalico/blox --json body -q .body 2>/dev/null | head -40 || true)
...
jq \
  ...
  --arg date "$DATE" \
  --arg notes "$NOTES" \
  --arg url "https://github.com/justacalico/blox/releases/download/$TAG/blox-ios-arm64-unsigned.ipa" \
  --argjson size "$SIZE" '
  .apps[0].versions = (
    [{
      version: $version,
