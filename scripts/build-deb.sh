...
mkdir -p "$PKGDIR/opt/blox" "$PKGDIR/usr/bin" "$PKGDIR/DEBIAN" \
  "$PKGDIR/usr/share/applications" \
  "$PKGDIR/usr/share/icons/hicolor/256x256/apps"

cp -r "$BUNDLE_DIR/." "$PKGDIR/opt/blox/"
ln -sf /opt/blox/blox "$PKGDIR/usr/bin/blox"
install -m 644 "$BUNDLE_DIR/data/flutter_assets/assets/icons/icon.png" \
  "$PKGDIR/usr/share/icons/hicolor/256x256/apps/blox.png"

cat > "$PKGDIR/usr/share/applications/blox.desktop" <<'EOF'
[Desktop Entry]
Type=Application
Name=Blox
